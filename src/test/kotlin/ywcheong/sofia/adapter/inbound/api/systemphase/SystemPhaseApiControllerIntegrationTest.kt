...
import ywcheong.sofia.adapter.outbound.persistence.systemphase.SystemPhaseJpaRepository
...
class SystemPhaseApiControllerIntegrationTest : IntegrationTestSupport() {
    ...
    @Test
    @DisplayName("시스템 단계 조회 - 데이터가 없으면 기본값(INACTIVE)을 반환한다")
    fun `GET system-phase returns default when not exists`() {
        mockMvc
            .perform(get("/admin/api/system-phase"))
            .andExpect(status().isOk)
            .andExpect(jsonPath("$.phaseType").value("INACTIVE"))
            .andExpect(jsonPath("$.startDate").value("1970-01-01T00:00:00Z"))
    }

    @Test
    @DisplayName("시스템 단계 변경 - 생성 및 업데이트가 정상 동작한다")
    fun `PUT system-phase creates and updates phase`() {
        mockMvc
            .perform(
                put("/admin/api/system-phase")
                    .contentType(MediaType.APPLICATION_JSON)
                    .content("""{"phaseType":"RECRUIT"}"""),
            ).andExpect(status().isOk)
            .andExpect(jsonPath("$.phaseType").value("RECRUIT"))

        mockMvc
            .perform(
                put("/admin/api/system-phase")
                    .contentType(MediaType.APPLICATION_JSON)
                    .content("""{"phaseType":"TRANSLATE"}"""),
            ).andExpect(status().isOk)
            .andExpect(jsonPath("$.phaseType").value("TRANSLATE"))

        assertThat(systemPhaseJpaRepository.count()).isEqualTo(1)
    }

    @Test
    @DisplayName("시스템 단계 변경 - startDate가 현재 시각으로 설정된다")
    fun `PUT system-phase populates startDate`() {
        mockMvc
            .perform(
                put("/admin/api/system-phase")
                    .contentType(MediaType.APPLICATION_JSON)
                    .content("""{"phaseType":"SETTLE"}"""),
            ).andExpect(status().isOk)
